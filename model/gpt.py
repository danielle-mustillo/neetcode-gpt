...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        # print("context shape", context.shape[1], "context length", self.context_length)
        x = self.word_embeddings(context)
        pos = torch.arange(self.context_length)
        # print("pos", pos)
        y = x + self.position_embeddings(pos)
        # print("y", y)
        # a = x + y
        # print("a", a)
        z = self.transformer(y)

        return self.vocabulary_projection(self.final_norm(z))
        pass
